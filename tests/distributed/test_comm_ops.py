...
def broadcast_tensor_dict_test_worker(tp_size: int, pp_size: int, rank: int,
    ...
    test_dict = {
        # device tensor
        "a": torch.arange(
            8, dtype=torch.float32, device=current_platform.device_name
        ),
        # CPU tensor
        ...
        # empty tensor
        "f": torch.tensor(
            [], dtype=torch.float32, device=current_platform.device_name
        ),
    }
...
def send_recv_test_worker(tp_size: int, pp_size: int, rank: int,
    ...
    test_tensor = torch.arange(
        64, dtype=torch.float32, device=current_platform.device_name
    )
